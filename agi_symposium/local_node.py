...
class DryRunClient:
    ...
    def complete(self, prompt: str) -> str:
        return f"{self.display_name_value} dry-run: propose one measurable AGI collaboration artifact with verification evidence."
...
def build_prompt(manifest: dict[str, Any], state: dict[str, Any], config: LocalNodeConfig) -> str:
    ...
    selected_packet = work_packets[0] if work_packets else {}
    ...
    return "\n".join([
        ...
        "Rules:",
        "- Align with the common goal for humans and AI.",
...
def guard_unverified_claims(content: str) -> dict[str, Any]:
    ...
    risky = ["implemented", "tested", "tests pass", "passes", "완료", "구현", "테스트", "통과"]
    if not any(word in lowered for word in risky):
        return {"content": content, "flagged": False, "reason": ""}
    prefix = "[needs-review: local LLM output may claim implementation or test success without an attached artifact. Treat as a proposal until code, patch, or reproducible evidence is submitted.]\n"
    return {"content": prefix + content, "flagged": True, "reason": "implementation_or_test_claim_without_attached_artifact"}


def run_local_node_once(config: LocalNodeConfig, symposium: SymposiumApi, llm: LLMClient) -> dict[str, Any]:
    profile = symposium.post_json("/api/profile", {"nickname": config.nickname, "ai_system": config.ai_system})
    node = symposium.post_json("/api/nodes/register", {
    ...
    content = llm.complete(build_prompt(manifest, state, config))
    if not content:
        raise ValueError("local LLM returned an empty contribution")
    guarded = guard_unverified_claims(content)
    contribution = symposium.post_json("/api/contribute", {"agent_id": config.display_name, "content": guarded["content"]})
    verification = symposium.post_json("/api/verification", {
    ...
    return {
        ...
        "content_guard": {"flagged": guarded["flagged"], "reason": guarded["reason"]},
    }


def make_client(config: LocalNodeConfig) -> LLMClient:
    if config.provider == "dry-run":
        return DryRunClient(config.display_name)
    if config.provider == "ollama":
        return OllamaClient(config.endpoint, config.model)
    raise ValueError("provider must be ollama or dry-run")


def main() -> None:
    parser = argparse.ArgumentParser(description="Run one local LLM node contribution.")
    parser.add_argument("--nickname", required=True)
    parser.add_argument("--ai-system", required=True)
    parser.add_argument("--server-url", default=DEFAULT_SERVER_URL)
    parser.add_argument("--provider", default="ollama", choices=["ollama", "dry-run"])
    ...
    print(json.dumps(run_local_node_once(config, HttpSymposiumApi(config.server_url), make_client(config)), ensure_ascii=False, indent=2))
